...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:key name="ptr-down" match="sect2"
		use="generate-id(preceding-sibling::sect1[1][not(@role)])"/>
	<xsl:key name="ptr-down" match="sect3"
		use="generate-id(preceding-sibling::sect2[1][not(@role)])"/>
	<xsl:key name="ptr-down" match="sect4"
		use="generate-id(preceding-sibling::sect3[1][not(@role)])"/>
	<xsl:key name="ptr-down" match="sect5"
		use="generate-id(preceding-sibling::sect4[1][not(@role)])"/>

	<xsl:key name="content"
		match="node()[not(self::chapter or self::sect1 or self::sect2 or self::sect3 or self::sect4 or self::sect5)]"
		use="generate-id((parent::content | preceding-sibling::*[self::chapter or self::sect1 or self::sect2 or self::sect3 or self::sect4 or self::sect5][1])[last()])"/>
	...
	<xsl:template match="chapter">
		<xsl:choose>
			<xsl:when test="normalize-space(.)='Preface'">
				<preface>
					<title>preface</title>
			...
			<xsl:otherwise>
				<xsl:copy>
					...
					<info>
						...
						<biblioid class="other" otherclass="dl">
							<xsl:value-of select="$codi"/>
						</biblioid>
						<biblioid class="other" otherclass="XML">
							<xsl:value-of select="$codi"/>
						</biblioid>
